...
/**
 * MemberRow — renders a single member with presence indicator and role badge.
 * Defined OUTSIDE MemberList to prevent recreation on every render cycle.
 */
function MemberRow({ member, getStatus }) {
...
export default function MemberList({ members = [], getStatus }) {
  ...
  return (
    <div className="h-full overflow-y-auto py-3">
      ...
      {online.length > 0 && (
        <div className="mb-3">
          <p className="px-3 text-[10px] font-semibold text-gray-300 uppercase tracking-wider mb-1">
            Online — {online.length}
          </p>
          {online.map((m) => (
            <MemberRow key={m.userId._id} member={m} getStatus={getStatus} />
          ))}
        </div>
      )}
